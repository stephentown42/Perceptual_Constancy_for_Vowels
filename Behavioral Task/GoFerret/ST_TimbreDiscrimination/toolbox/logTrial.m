...
if ischar(response), % Enter string as arguement to get header
    ...
    headings = {'Trial','CorrectionTrial?','StartTime','CenterReward?','F1','F2','F3','F4','HoldTime','Atten',...
        ...
        if isfield(gf,'spatialUnmasking'),
            fprintf(gf.fid,'SpatialMask\t');
        end
...
%     end
    ...
    output = {'gf.TrialNumber'          ,'%d'   ;
        ...
        if isfield(gf,'mask'),
            if isfield(gf,'reportMask')
                fprintf(gf.fid,'%.3f\t', gf.reportMask);
            else            
                fprintf(gf.fid,'%d\t', gf.mask);
            end
        end
