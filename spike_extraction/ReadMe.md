...
### Data Organization Prior to Spike Sorting
It is possible to go straight from event dection to spike sorting in MClust. However in practice, we often record neural activity from the same electrodes in the same positions on multiple recording sessions. In the spike sorting process, we may thus want to combine spikes across sessions to reduce the amount of work required during sorting. 

## 4. Spike Sorting in MClust

### Input Files
Spike sorting procedes one channel at a time, using the individual .mat files generated by the event detection stage described above.
...
The spike times and behavioral timestamps are sufficient to do quick inspection of data for things like sound driven activity (see Data Visualization). Although it is possible to go straight to the analysis of spike times, it is wise to perform additional quality checks, before 