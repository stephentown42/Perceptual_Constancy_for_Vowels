...
if ProjectID ~= -13
    ...
    % Warn user
    error('You have failed to select the correct project and circuit - further failure will not be tolerated')
end
...
% Make a new tab delimited file 
    ...
    logName   = sprintf('%02d_%02d_%02d %s %02d_%02d_%s %s.txt',...
                            t(3), t(2), t(1), paramFile,...
                            t(4), t(5), ss, gf.recBlock);      % Time Filename log.txt   
...
% Initialize raster
rstr.edges      = -0.250 : 0.001 : 1.000;
rstr.nEdges     = length(rstr.edges);
rstr.nTrials    = 200;
rstr.startTimes = [];
rstr.sev        = zeros(32,1);
rstr.tev        = cell(32,1);
...
for i = 1 : 6, 
    bI       = (i-1)*0.25 - 0.25;
    xtick(i) = find(rstr.edges == bI);          %#ok<*AGROW>
    xlabl{i} = sprintf('%.2f', bI);
end

axes(h.rasterAxes)
colormap('gray')

rstr.im = imagesc(rand(rstr.nTrials,rstr.nEdges));

set(h.rasterAxes,...
            'xtick',xtick,...
            'xticklabel',xlabl,...
            'xlim',[0 rstr.nEdges],...
            'ylim',[0.5 rstr.nTrials],...
            'FontSize',7,...
            'XLimMode','manual',...
            'YLimMode','manual',...
            'ZLimMode','manual',...
...
% Default waveform graph paramerters
% gf.waveformChan  = get(h.drawChan,'value');
% gf.waveformStore = get(h.drawStore,'value');
gf.ssnip         = 0;
