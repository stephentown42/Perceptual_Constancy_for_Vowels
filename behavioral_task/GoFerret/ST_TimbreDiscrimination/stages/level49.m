...
try
% Level 9 
% Makes probe trials whispers (unvoiced) using white noise rather than
% pulse train
...
%__________________________________________________________________________    
    case('PrepareStim')
        ...
        % Stimulus grid for vowel type
        % Intialize variables if don't exist
        if ~isfield(gf,'sideIdx'), gf.sideIdx = 1; end
        if ~isfield(gf,'sideList'), 
            gf.sideList = [ones(1,3) zeros(1,3) 2 3];                          
            gf.sideList = gf.sideList( randperm( length(gf.sideList))); 
        end
            
            % Reset variables when index is too large
        if gf.sideIdx > length(gf.sideList),
            gf.sideList = gf.sideList( randperm( length(gf.sideList)));
            gf.sideIdx = 1;
        end
        
        % Get side
        gf.side    = gf.sideList( gf.sideIdx);
        gf.sideIdx = gf.sideIdx + 1;
        
        gf.formants = eval(sprintf('gf.sound%d',gf.side));
        
        % reward either side for mismatch trials
        if gf.side > 1, gf.side = -1; end
...
%         gf.formants = eval(sprintf('gf.sound%d',sideIdx - 1));
        ...
        gf.formants = [437,2761,2975,4263];
            %460,1105,2857,4205];
% sound1	[730,2058,2857,4205]
% sound2	[936,1551,2975,4263]
% sound3	[437,2761,2975,4263]
        ...
        gf.pitch       = NaN;
        gf.atten       = min(gf.attenRange);
        gf.duration = 1000;
        %             if ismember(gf.formants,[460 1105 2857 4205],'rows'), gf.atten = gf.atten - 5; end
%             if ismember(gf.formants,[730 2058 2857 4205],'rows'), gf.atten = gf.atten - 3.5; end

        sound  = ComputeTimbreStimWhisper_NoEnvelope(gf.formants)';
...
%         sound  = [sound, sound, sound];                        % create two vowels with two intervals
       ...
       % Calibrate sounds
       sound0 = sound .* 10 ^(-(40/20));
       sound1 = sound .* 10 ^(-(40/20));
        ...
        if ~strcmp( datestr(now,'dd-mm-yy'), '02-08-14') 
...
% Center Response__________________________________________________________        
    case('WaitForStart')
        ...
        if centerLick == 0;
            ...
            if gf.centerRewardP > rand(1) && gf.side >= 0,
                gf.centerReward = 1;
...
%        gf.responseTime = DA.GetTargetVal(sprintf('%s.lickTime',gf.stimDevice)) ./ gf.fStim;  %Open Ex
            ...
            if rightLick > 0,
                ...
                if gf.side == 1 || gf.side == -1 || gf.side == 3, % Correct response
            ...
            elseif leftLick > 0 
                ...
                if gf.side == 0 || gf.side == -1 || gf.side == 2,
